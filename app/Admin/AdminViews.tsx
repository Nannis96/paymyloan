// app/admin/AdminViews.tsx
"use client";

import { useSite } from "@/app/components/layout/SiteShell";

// Helpers para formatear dinero
          ...
          <table className="w-full text-left">
            ...
            <tbody className="divide-y divide-rule text-[12px] text-ink-2">
              {contracts.length > 0 ? contracts.map((deal, i) => (
                ...
                <tr><td colSpan={4} className="p-6 text-center text-ink-3">No hay tratos registrados.</td></tr>
              )}
            </tbody>
          </table>
        </div>
          ...
          <table className="w-full text-left">
            ...
            <tbody className="divide-y divide-rule text-[12px] text-ink-2">
              {lenders.length > 0 ? lenders.map((u, i) => (
                ...
                <tr><td colSpan={3} className="p-6 text-center text-ink-3">No hay prestamistas registrados.</td></tr>
              )}
            </tbody>
          </table>
        </div>
...
export function BorrowersView({ users }: { users: any[] }) {
  const { t, lang } = useSite();
  const d = t.dashboardAdmin;
  const v = d.views.borrowers;
      ...
      <div className="overflow-hidden rounded-[10px] border border-rule bg-surface shadow-sm">
        <div className="overflow-x-auto">
          <table className="w-full text-left">
            ...
            <tbody className="divide-y divide-rule text-[12px] text-ink-2">
              {borrowers.length > 0 ? borrowers.map((u, i) => (
                <tr key={u.id || i} className="hover:bg-surface-2 transition-colors">
                  <td className="px-4 py-3 font-bold text-ink">{u.name}</td>
                  <td className="px-4 py-3">{u.createdAt ? new Date(u.createdAt).toLocaleDateString(lang === "es" ? "es-MX" : "en-US", { month: "short", day: "numeric" }) : "N/A"}</td>
                  <td className="px-4 py-3">
                ...
                <tr><td colSpan={3} className="p-6 text-center text-ink-3">No hay prestatarios registrados.</td></tr>
              )}
            </tbody>
          </table>
        </div>
